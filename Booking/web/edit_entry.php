...
function create_field_entry_confirmation_status($disabled=FALSE)
{
  ...
  if ($confirmation_enabled)
  {
    ...
    $buttons[0] = get_vocab("暫定");
    $buttons[1] = get_vocab("確認");
    
    $params = array('label'       => get_vocab("確認狀態") . ":",
                    'name'        => 'confirmed',
                    'value'       => ($confirmed) ? 1 : 0,
                    'options'     => $buttons,
                    'force_assoc' => TRUE,
...
if (($edit_type == "series") && $repeats_allowed)
{
 ...
 echo "</fieldset>\n";
    ...
    echo "<input class=\"submit\" type=\"submit\" name=\"back_button\" value=\"" . get_vocab("返回") . "\" formnovalidate>\n";
    echo "</div>\n";
